function Experience() {
  ...
  const filteredFaculties = faculties.filter((f) => {
    let exp = f.FacultyExperience.split("+")[0];
    return exp > 10;
  });

  const formatedFac = filteredFaculties.map((f) => {
    return (
      <div className="card col-md-3 m-3 text-center">
        <img src={f.FacultyImage} className="card-img-top" alt="..." />
        <div className="card-body">
          <h2 className="card-title"> {f.FacultyName} </h2>
        </div>
      </div>
    );
  });

  return <div className="row justify-content-center"> {formatedFac} </div>;
}
